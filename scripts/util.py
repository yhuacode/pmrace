...
class ProgramError(Exception):
    def __init__(self, message, outs, errs):
        self.message = message
        self.outs = outs
        self.errs = errs

    def __str__(self):
        return 'ProgramError: {}'.format(self.message)
    ...
    with subprocess.Popen(
        ...
        try:
            ...
            if rc != 0:
                ...
                message = 'Failed to execute {}: exit code {}'.format(
                    ' '.join(cmd), rc
                )
                raise ProgramError(message, outs, errs)

            return outs, errs
        except subprocess.TimeoutExpired:
            p.kill()

            message = 'Failed to execute {}: timed out'.format(' '.join(cmd))
